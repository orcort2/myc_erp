...
export default function TechnicianHome() {
  ...
  useEffect(() => {
    ...
    authorizedFetch(apiUrl('/mobile/v1/technician/tickets?status=pending&limit=25'))
      .then(async (response) => response.ok ? response.json() as Promise<OperationalTicket[]> : [])
      ...
      <View style={styles.content}>
        ...
        {hasPermission(user.permissions, 'tickets.view_own') && (
          <Pressable style={styles.module} onPress={() => router.push('/(technician)/tickets')}>
            ...
            <Text style={styles.moduleText}>{pendingTickets === null ? 'Solicitudes operativas' : `${pendingTickets} pendiente${pendingTickets === 1 ? '' : 's'}`}</Text>
          </Pressable>
        )}
