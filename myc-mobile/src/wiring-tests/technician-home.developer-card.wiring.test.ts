...
/**
 * DEV-0: the "Desarrollador" Home card is offered only when the current
 * user's permissions include developer.access, and it navigates to the
 * Developer screen -- nothing about Developer's own lock state is decided
 * here. Executes the real technician/index.tsx source; only its
 * dependencies are mocked, same convention as the sibling wiring tests.
 */
...
function harness(canAccessDeveloper: boolean) {
  ...
  const ports: Record<string, unknown> = {
    ...
    'expo-router': { Redirect: 'Redirect', router: { push: (path: string) => { navigations.push(path); }, replace: () => undefined } },
    ...
    '@/src/permissions/mobile-capabilities': {
      deriveMobileCapabilities: () => ({
        canAccessDeveloper, canCaptureFieldSheets: false, canClaimWorkOrderGroupRequests: false,
        canCreateWorkOrders: false, canReadLabClients: false, canReadTickets: false,
        canReadWorkOrderGroupRequests: false, canReadWorkOrders: false, canReviewTickets: false,
        canUseCommunications: false,
      }),
    },
...
test('sin developer.access, Inicio no ofrece la tarjeta Desarrollador', () => {
  const app = harness(false);
  const tree = app.render();
  assert.equal(pressableByText(tree, 'Desarrollador'), undefined);
});

test('con developer.access, Inicio ofrece la tarjeta Desarrollador', () => {
  const app = harness(true);
  const tree = app.render();
  const card = pressableByText(tree, 'Desarrollador');
  assert.ok(card, 'esperaba la tarjeta Desarrollador');
});

test('tocar la tarjeta Desarrollador navega a /(technician)/developer', () => {
  const app = harness(true);
  const tree = app.render();
  const card = pressableByText(tree, 'Desarrollador')!;
  (card.props!.onPress as () => void)();
  assert.deepEqual(app.navigations, ['/(technician)/developer']);
});
