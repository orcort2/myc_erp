...
test('12. received_signed oculta editar equipo (editable gatea abrir el editor de equipo)', () => {
  const source = screenSource();
  // PENDIENTE 3 (encargo de corrección LAB): "Editar datos" pasó a ser una
  // acción explícita, independiente del tap ambiguo de la fila (antes, en
  // equipment_by_equipment, el tap SIEMPRE navegaba a captura técnica y
  // JAMÁS ofrecía editar). La garantía real (editable && canManageEquipment
  // sigue gateando showEquipmentEditor) es la misma; sólo cambió de forma --
  // ahora vía la variable intermedia canEditThisEquipment.
  assert.match(source, /const canEditThisEquipment = editable && canManageEquipment;/);
  assert.match(source, /\{canEditThisEquipment && \([\s\S]{0,120}showEquipmentEditor\(item\)/);
});
