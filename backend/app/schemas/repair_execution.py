...
class RepairEquipmentCreate(BaseModel):
    ...
    def require_existing_or_name(self):
        if self.equipment_id is None and not self.name:
            raise ValueError("Se requiere equipment_id o nombre para dar de alta el equipo")
        return self
...
class RepairDiagnosis(BaseModel):
    """Decisión funcional #3: diagnóstico opcional. Este endpoint es opcional en el flujo."""

    diagnosis_notes: str = Field(min_length=3, max_length=3000)


class RepairConclude(BaseModel):
    """Decisión funcional #4: conclusión 'equipment_not_suitable' exige justificación."""

    conclusion: Literal["repaired", "equipment_not_suitable"]
    conclusion_reason: str | None = Field(default=None, max_length=2000)

    @model_validator(mode="after")
    def require_reason_when_not_suitable(self):
        if self.conclusion == "equipment_not_suitable" and (
            not self.conclusion_reason or len(self.conclusion_reason.strip()) < 10
        ):
            raise ValueError(
                "La conclusión 'equipment_not_suitable' requiere una justificación de al menos 10 caracteres"
            )
        return self


class RepairInterventionCreate(BaseModel):
    description: str = Field(min_length=3, max_length=3000)
    actions: list[dict] = Field(default_factory=list, max_length=200)
    removed_components: list[dict] = Field(default_factory=list, max_length=100)
    outcome: Literal["effective", "partial", "ineffective"] | None = None

    @model_validator(mode="after")
    def validate_removed_components(self):
        for component in self.removed_components:
            disposition = component.get("disposition", "return_to_client")
            if disposition not in ("return_to_client", "client_authorized_disposal"):
                raise ValueError(
                    "removed_components.disposition debe ser 'return_to_client' o 'client_authorized_disposal'"
                )
            if not component.get("name"):
                raise ValueError("removed_components requiere 'name'")
        return self
...
class RepairPauseCreate(BaseModel):
    pause_type: Literal["spare_part", "authorization", "client_decision", "administrative_investigation"]
...
class RepairChangeCreate(BaseModel):
    change_type: Literal["additional_scope", "investigation"]
    summary: str = Field(min_length=3, max_length=3000)


class RepairChangeResolve(BaseModel):
    decision: Literal["approved", "rejected", "linked"]
    reason: str = Field(min_length=3, max_length=2000)
    quotation_item_id: int | None = Field(default=None, gt=0)
    linked_service_order_id: int | None = Field(default=None, gt=0)


class RepairSignature(BaseModel):
    """Espejo exacto de MaintenanceSignature (app/schemas/maintenance_execution.py)."""

    signer_name: str = Field(min_length=2, max_length=180)
    signature_data_url: str = Field(max_length=350_000)
    client_decision: Literal["accepted", "rejected_additional_work", "acknowledged"]
    ...
    def validate_signature(self):
        ...
        if match.group(1) == "png" and not binary.startswith(b"\x89PNG\r\n\x1a\n"):
            raise ValueError("La firma PNG no es válida")

        if match.group(1) == "jpeg" and not binary.startswith(b"\xff\xd8\xff"):
            raise ValueError("La firma JPEG no es válida")
...
class RepairWarrantyReopen(BaseModel):
    """Decisión funcional #12: preparable. Reabre un ciclo sin borrar el cierre histórico."""
...
class RepairInterventionRead(RepairEntityRead):
    sequence: int
    technician_id: int
    description: str
    actions: list
...
class RepairTestRead(RepairEntityRead):
    sequence: int
...
class RepairPauseRead(RepairEntityRead):
    pause_type: str
    reason: str
...
class RepairChangeRead(RepairEntityRead):
    change_type: str
    summary: str
    status: str
...
class RepairExecutionRead(RepairEntityRead):
    ...
    origin: str
    source_maintenance_change_request_id: int | None
    ...
    pauses: list[RepairPauseRead]
    interventions: list[RepairInterventionRead]
    tests: list[RepairTestRead]
    changes: list[RepairChangeRead]
...
class RepairBoardRead(BaseModel):
    service_order_id: int
    executions: list[RepairExecutionRead]
    blockers: list[dict]
    can_close: bool
