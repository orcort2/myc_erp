...
MaintenanceStatus = Literal[
    "pending_arrival", "received", "pending_assignment", "assigned",
    "in_maintenance", "paused", "technically_completed", "pending_release", "closed",
]
...
class MaintenanceCapture(BaseModel):
    initial_condition: Literal["operational", "operational_with_anomalies", "not_operational", "undetermined"]
    initial_description: str = Field(min_length=3, max_length=2000)
    findings: list[dict] = Field(default_factory=list, max_length=100)
    actions: list[dict] = Field(default_factory=list, max_length=200)
    final_condition: Literal["operational", "operational_with_observations", "not_operational", "requires_additional_intervention"]
    functional_result: str = Field(min_length=3, max_length=3000)
    technical_conclusion: str = Field(min_length=3, max_length=2000)
    recommendations: list[dict] = Field(default_factory=list, max_length=100)
    before_photos: list[str] = Field(default_factory=list, max_length=20)
    after_photos: list[str] = Field(default_factory=list, max_length=20)

    @model_validator(mode="after")
    def validate_photo_references(self):
        for reference in [*self.before_photos, *self.after_photos]:
            if len(reference) > 500 or reference.startswith(("/", "data:", "http://", "https://", "file:")) or ".." in reference.split("/"):
                raise ValueError("Las fotografías deben usar referencias relativas seguras del almacenamiento institucional")
        return self


class MaintenancePauseCreate(BaseModel):
    pause_type: Literal["spare_part", "authorization", "second_intervention", "commercial_review", "administrative_investigation"]
...
class MaintenanceMaterialCreate(BaseModel):
    material_type: Literal["used", "required"]
    name: str = Field(min_length=1, max_length=180)
    quantity: Decimal = Field(gt=0)
    unit: str = Field(min_length=1, max_length=40)
    component: str | None = Field(default=None, max_length=180)
    notes: str | None = Field(default=None, max_length=2000)
    internal_unit_cost: Decimal | None = Field(default=None, ge=0)
    decision: Literal["accepted", "rejected", "pending"] | None = None


class MaintenanceChangeCreate(BaseModel):
    change_type: Literal["corrective", "repair", "investigation"]
    summary: str = Field(min_length=3, max_length=3000)


class MaintenanceChangeResolve(BaseModel):
    decision: Literal["approved", "rejected", "overridden", "linked"]
    reason: str = Field(min_length=3, max_length=2000)
...
class MaintenancePauseRead(MaintenanceEntityRead):
    pause_type: str
    reason: str
...
class MaintenanceMaterialRead(MaintenanceEntityRead):
    material_type: str
    name: str
    quantity: Decimal
    unit: str
...
class MaintenanceChangeRead(MaintenanceEntityRead):
    change_type: str
    summary: str
    status: str
...
class MaintenanceExecutionRead(MaintenanceEntityRead):
...
class MaintenanceBoardRead(BaseModel):
    service_order_id: int
    executions: list[MaintenanceExecutionRead]
    blockers: list[dict]
    can_close: bool
