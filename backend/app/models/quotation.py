...
class QuotationItem(
    ...
    # Identidad operacional congelada en la partida.
    #
    # No debe inferirse nuevamente desde item_type, commodity,
    # nombre o catálogo vigente al crear el ETS.
    ...
    # Snapshot técnico/comercial de la partida al momento de cotizar.
    operational_snapshot: Mapped[dict | None] = mapped_column(
        JSON,
    )
    ...
    technical_request_id: Mapped[int | None] = mapped_column(
        ForeignKey(
            "technical_service_requests.id",
            ondelete="RESTRICT",
        ),
        index=True,
    )

    equipment_snapshot: Mapped[dict | None] = mapped_column(
        JSON,
    )

    tax_object: Mapped[str | None] = mapped_column(
        String(20),
    )
...
class QuotationItemDecision(
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "quotation_item_id",
            name="uq_quotation_item_decisions_item",
        ),
    )
    ...
    enabled_stage_categories: Mapped[list] = mapped_column(
        JSON,
        default=list,
    )
...
class QuotationSnapshot(
    ...
    snapshot_number: Mapped[int] = mapped_column(
        default=1,
    )
