...
from app.models.service_execution import ServiceStage
...
def ctx():
    ...
    admin = User(
        ...
        email="maint-admin@example.test",
        full_name="Admin Mantenimiento",
    ...
    advisor = User(
        username="maint-advisor",
        email="maint-advisor@example.test",
        full_name="Asesor Mantenimiento",
        hashed_password="unused",
    ...
    client = Client(
        legal_name="Cliente Mantenimiento"
    )
...
def _catalog(
    ...
    *,
    ...
    item = CatalogItem(
        ...
        tax_object="iva_16",
        tax_rate=Decimal("16"),
        calibration_scope=maintenance_type,
        maintenance_type=maintenance_type,

        # Este dato permanece únicamente para demostrar
        # que el ETS ya no debe heredar la modalidad
        # operativa desde el catálogo.
    ...
    *,
    ...
    return MaintenanceCapture(
        ...
        technical_conclusion=(
            "Intervención concluida según alcance"
        ),
        recommendations=(
            recommendations or []
        ),
        before_photos=[
            "maintenance/before-1.jpg"
        ],
        after_photos=[
...
def _start_lab(
    ...
    # La modalidad ya no nace desde Catálogo.
    # Se define al asignar la ejecución.
    prepare_execution(
        db,
        order.id,
        execution.id,
...
def test_preventive_laboratory_birth_freezes_snapshot_and_materializes_ot_units(
    ctx,
):
    ...
    # La modalidad operativa pertenece a cada
    # ejecución y no al concepto comercial.
    assert {
        item.location_mode
        for item in executions
    } == {None}
    ...
    assert all(
        item.service_unit.evolution_enabled
        is False
        for item in executions
    )

    assert all(
        item.service_unit.work_order_id
        for item in executions
    )

    # Cambiar posteriormente el catálogo tampoco
    # debe modificar tipo ni modalidad de los ETS.
    catalog.maintenance_type = "corrective"
    catalog.maintenance_location = "field"
    db.commit()
    ...
    MaintenanceBoardRead.model_validate(
        board
    )

    assert {
        item["maintenance_type"]
        for item in board["executions"]
    } == {"preventive"}
...
def test_corrective_catalog_base_material_is_frozen_without_exposing_cost(
    ctx,
):
    db, _, _, advisor, client = ctx
...
def test_field_has_no_arrival_and_requires_equipment_request_acceptance_and_schedule(
    ctx,
):
    db, _, technician, advisor, client = ctx

    # Aunque el catálogo diga "field", el ETS
    # debe nacer sin modalidad operativa.
    order = _order(
        db,
        client,
        advisor,
    ...
    assert execution.location_mode is None

    # Modalidad y técnico se definen por ejecución.
    prepare_execution(
        db,
        order.id,
        execution.id,
    ...
    # Todavía no existe equipo vinculado.
    ...
    # Tener equipo tampoco permite iniciar:
    # falta aceptación/programación de campo.
    with pytest.raises(HTTPException):
        start_execution(
            db,
            order.id,
    ...
    assert (
        execution.scheduled_for.replace(
            tzinfo=timezone.utc,
        )
        == scheduled
    )
...
def test_typed_spare_part_pause_and_second_visit_are_traceable(
    ctx,
):
    db, _, technician, advisor, client = ctx
...
def test_preventive_corrective_requires_own_approved_commercial_link(
    ctx,
):
...
def test_rejected_corrective_remains_documented_and_does_not_block_preventive_close(
    ctx,
):
    db, admin, technician, advisor, client = ctx
    ...
    resolve_change(
        ...
        MaintenanceChangeResolve(
            decision="rejected",
            reason=(
                "Cliente decide no ampliar alcance"
            ),
        ),
        actor=admin,
    )
...
def test_material_used_and_required_are_separate_and_report_has_no_internal_cost(
    ctx,
):
    db, _, technician, advisor, client = ctx
    ...
    add_material(
        ...
        MaintenanceMaterialCreate(
            ...
            quantity=Decimal("1"),
            unit="pieza",
            internal_unit_cost=Decimal("900"),
            decision="pending",
        ),
        actor=technician,
    )
    ...
    original = service.HTML
    service.HTML = FakeHTML
...
def test_repair_is_only_requested_and_never_executed_inside_maintenance(
    ctx,
):
    db, admin, technician, advisor, client = ctx
    ...
    assert all(
        stage.category != "repair"
        for stage in stages
    )


def test_inoperable_creates_administrative_block_and_linked_investigation(
    ctx,
):
    db, admin, technician, advisor, client = ctx
...
def test_maintenance_and_calibration_coexist_as_independent_units(
    ctx,
):
    db, _, _, advisor, client = ctx
    ...
    calibration = CatalogItem(
        ...
        name="Calibración",
        origin_price=Decimal("50"),
        origin_currency="MXN",
        exchange_rate=Decimal("1"),
        margin_percent=Decimal("0"),
        final_price_mxn=Decimal("50"),
        tax_object="iva_16",
        tax_rate=Decimal("16"),
    ...
    assert (
        execution.service_unit.initial_category
        == "maintenance"
    )
    ...
    assert (
        execution.service_unit.evolution_enabled
        is False
    )


def test_technical_completion_report_signature_and_close_are_distinct(
    ctx,
):
    db, admin, technician, advisor, client = ctx
