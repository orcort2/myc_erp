...
import { getFieldSheetTemplate } from '../constants/fieldSheetTemplates.js';

function buildDefaultResultsRows(templateKey = 'general') {
  return getFieldSheetTemplate(templateKey).resultSections.flatMap((section) =>
...
export function buildFieldSheetResultSections(rows = [], templateKey = 'general') {
  const template = getFieldSheetTemplate(templateKey);
  const normalizedRows = normalizeResultsRows(rows, templateKey);
  return template.resultSections.map((section) => ({
    key: section.key,
    title: section.title,
