...
const statusLabels = {
  ...
  warranty_return: 'Retornado por garantía', replaced: 'Sustituida por garantía', delivered: 'Entregada', resolved: 'Resuelta comercialmente',
...
export default function SaleEtsTab({ order, user = null, users = [] }) {
  ...
  const [warrantyResolution, setWarrantyResolution] = useState({ unitId: null, resolution: 'return_to_flow', reason: '' });
  ...
  return (
    <section className="sale-ets">
                  ...
                  {item.units.map((unit, index) => (
                    <div className="sale-unit" key={unit.id}>
                      ...
                      <div className="toolbar-actions">
                        ...
                        {unit.arrived_at && !['delivered', 'resolved', 'replaced', 'warranty_return'].includes(unit.status) ? <button className="table-button table-button--danger" onClick={() => setWarranty({ unitId: unit.id, reason: '' })} type="button">Garantía</button> : null}
                        {canAuthorize && unit.status === 'warranty_return' ? <button className="table-button" onClick={() => setWarrantyResolution({ unitId: unit.id, resolution: 'return_to_flow', reason: '' })} type="button">Resolver garantía</button> : null}
      ...
      {warrantyResolution.unitId ? <form className="sale-panel" onSubmit={(event) => { event.preventDefault(); mutate(() => resolveSaleWarranty(order.id, warrantyResolution.unitId, { resolution: warrantyResolution.resolution, reason: warrantyResolution.reason }), 'Garantía resuelta con trazabilidad comercial.'); setWarrantyResolution({ unitId: null, resolution: 'return_to_flow', reason: '' }); }}><h4>Resolución administrativa de garantía</h4><div className="sale-form-grid"><label>Resultado<select onChange={(event) => setWarrantyResolution({ ...warrantyResolution, resolution: event.target.value })} value={warrantyResolution.resolution}><option value="return_to_flow">Unidad vuelve al flujo</option><option value="replacement">Reemplazo pendiente de arribo</option><option value="commercial_cancellation">Cancelación comercial definitiva</option></select></label><label>Justificación<textarea minLength="3" onChange={(event) => setWarrantyResolution({ ...warrantyResolution, reason: event.target.value })} required value={warrantyResolution.reason} /></label></div><div className="toolbar-actions"><button className="primary-button" disabled={busy} type="submit">Aplicar resolución</button><button className="table-button" onClick={() => setWarrantyResolution({ unitId: null, resolution: 'return_to_flow', reason: '' })} type="button">Cancelar</button></div></form> : null}
