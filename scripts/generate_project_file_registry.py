...
def classify(path: Path) -> tuple[str, str, str, str, str]:
    ...
    work_order_deletion_files = {
        ...
        "backend/app/routers/lab_work_orders.py": ("API OT LAB", "Expone cierre/firma grupal o individual, cambio de modalidad, preview/distribución de folios certificado pendientes y DELETE administrativo con permisos/scope Mobile, sin reutilizar rutas productivas.", "FastAPI, auth, schemas y servicio LAB", "MYC Mobile y pruebas LAB", "Crítico"),
        "backend/app/services/lab_work_orders.py": ("Autoridad OT LAB", "Gestiona folios (bloqueando accredited/traceable externo sin pool resuelto y reparando pendientes legacy vía Distribuir folios disponibles), grupo histórico, cohortes grupales/individuales, inmutabilidad completed, PDFs y eliminación atómica con reparación de cadena y recursos compartidos.", "Modelos LAB, Tickets, Notifications, Communications, auditoría y PDFs", "Router LAB, MYC Mobile y pruebas", "Crítico"),
        "backend/tests/test_lab_work_orders.py": ("Suite integral OT LAB", "Cubre lifecycle, cohortes, reapertura/eliminación y la importación LabClient histórica/estructurada con aliases, auxiliares ignorados, ceros iniciales, inválidos y duplicados.", "Pytest, FastAPI, openpyxl, SQLAlchemy y agregado LAB", "Gate backend del vertical temporal", "Crítico"),
        "myc-mobile/app/(technician)/work-orders.tsx": ("Flujo OT LAB móvil", "Distingue grupo histórico y cohorte, evita recapturar firmas históricas preservadas durante CTA/open/realtime/navegación relacionada y restablece el flujo normal al invalidarse; ofrece cierre grupal/individual, explica faltantes y navega estado/PDF por folio.", "AuthProvider, API LAB, permisos, política/servicio de firma, Notifications y Expo", "Técnicos y Administrador móvil", "Crítico"),
    ...
    lab_operational_ux_files = {
        ...
        "myc-mobile/app/(technician)/work-orders.tsx": ("Flujo móvil OT LAB", "Orquesta datos/equipos/firmas/captura/cierre, errores por campo, fechas y acciones con primitives canónicos.", "API LAB, capabilities, firma, calendario y componentes", "Staff y técnicos autorizados", "Crítico"),
        ...
        "myc-mobile/src/wiring-tests/work-orders.operational-ux.wiring.test.ts": ("Regresión UX OT LAB", "Verifica canon de botones y cableado de fieldErrors en datos generales/equipo.", "node:test y fuente work-orders", "Gate Mobile", "Alto"),
    ...
    lab_files = {
        ...
        "backend/app/routers/lab_work_orders.py": ("API móvil OT LAB", "Expone filtros/paginación, CRUD, adicionales, firma/cierre grupal o individual, PDF actual e historial/PDF por revisión bajo permisos explícitos.", "FastAPI, schemas, servicios LAB/Tickets y auth Mobile", "myc-mobile y exportación administrativa", "Crítico"),
        ...
        "myc-mobile/app/(technician)/work-orders.tsx": ("Flujo móvil OT LAB", "Presenta grupo histórico y estado por folio; elige cierre grupal o individual, conserva captura por contexto de cohorte y abre el PDF de cada OT sin alterar parentesco.", "Expo Router, AuthProvider, Notifications, política de cohorte, firma y API LAB", "Técnicos autorizados en Expo Go/TestFlight", "Crítico"),
