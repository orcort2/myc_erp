...
def classify(path: Path) -> tuple[str, str, str, str, str]:
    ...
    maintenance_files = {
        ...
        "backend/app/security/api_access.py": ("Política transversal de acceso", "Clasifica 463 operaciones deny-by-default, incluidos los límites ETS, Mobile y administrativos vigentes.", "FastAPI, permisos e inventario CSV", "Middleware, arranque, generador y pruebas", "Crítico"),
    ...
    work_order_deletion_files = {
        ...
        "backend/app/security/api_access.py": ("Política transversal de acceso", "Clasifica 463 operaciones deny-by-default y separa permisos internos, Portal y Mobile.", "FastAPI, auth, catálogo de permisos e inventario CSV", "Middleware, arranque, generador y pruebas de conformidad", "Crítico"),
        ...
        "backend/tests/test_api_access_conformity.py": ("Prueba de conformidad API", "Exige clasificación de las 463 operaciones y coincidencia exacta entre runtime e inventario CSV.", "app.main, security.api_access e inventario CSV", "Suite backend y revisión de API", "Crítico"),
        ...
        "backend/app/services/lab_work_orders.py": ("Autoridad OT LAB", "Gestiona folios 6400–6999, grupos directos/solicitados, equipos, firmas, PDFs y eliminación atómica con reparación de cadena, recursos compartidos y solicitud histórica.", "Modelos LAB, Tickets, Notifications, Communications, auditoría y PDFs", "Router LAB, MYC Mobile y pruebas", "Crítico"),
        "backend/tests/test_lab_work_orders.py": ("Suite integral OT LAB", "Cubre lifecycle LAB, folios, grupos directos/solicitados, firmas, PDFs, Tickets y eliminación administrativa de adicional/raíz/última con rollback, reparentado histórico y folios no reutilizables.", "Pytest, FastAPI, SQLAlchemy y agregado LAB", "Gate backend del vertical temporal", "Crítico"),
        ...
        "docs/architecture/security/API_ENDPOINT_INVENTORY_2026-08-03.csv": ("Evidencia reproducible API", "Registra 463 operaciones con método, ruta, identidad, permiso, ownership y pruebas esperadas.", "generate_api_access_inventory.py y app runtime", "Conformidad, revisión externa y CI futura", "Crítico"),
    }
