...
def classify(path: Path) -> tuple[str, str, str, str, str]:
    ...
    field_sheet_dsl_files = {
        ...
        "backend/app/services/field_sheet_layouts.py": ("Normalizador visual de Hojas de Campo", "Valida geometría matricial y posición lógica de column_key, longitudes allowlisted y print_layout; resuelve perfiles MYC/CAPYMET aislando su contacto e identidad.", "Schemas Pydantic y HTTPException", "Motor de plantillas y renderer PDF", "Crítico"),
        ...
        "backend/app/services/field_sheet_pdfs.py": ("Autoridad documental PDF", "Compone contexto limpio de página, bloques, campos, tablas multinivel, row labels, perfiles organizacionales aislados y firmas para el renderer único versionado.", "FieldSheet, layouts, Jinja/WeasyPrint, snapshots y storage", "Endpoints PDF, paquetes y cierres FieldSheet", "Crítico"),
        "backend/app/templates/field_sheet_engine_pdf.html": ("Renderer HTML canónico", "Presenta layout fluido declarativo, perfiles MYC/CAPYMET, grids, headers con colspan/rowspan, row labels y cortes v1 compatibles sin lógica por template.", "Contexto field_sheet_pdfs, Jinja y WeasyPrint", "PDFs nuevos ERP/LAB y snapshots v1 compatibles", "Crítico"),
        "backend/tests/test_field_sheet_layout_dsl.py": ("Aceptación DSL/PDF Hojas de Campo", "Prueba defaults v1, headers planos/agrupados, spans, posición lógica/422, row labels, aislamiento CAPYMET, seguridad y PDFs de fixtures temporales.", "Pytest, normalizador, Jinja y WeasyPrint", "Gate Fases 4+5", "Crítico"),
