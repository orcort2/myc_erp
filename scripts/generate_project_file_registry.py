...
def classify(path: Path) -> tuple[str, str, str, str, str]:
    ...
    maintenance_files = {
        ...
        "backend/app/security/api_access.py": ("Política transversal de acceso", "Clasifica 519 operaciones deny-by-default, incluida regeneración PDF administrativa con field_sheets.review.", "FastAPI, permisos e inventario CSV", "Middleware, arranque, generador y pruebas", "Crítico"),
    ...
    work_order_deletion_files = {
        ...
        "backend/app/security/api_access.py": ("Política transversal de acceso", "Clasifica 519 operaciones deny-by-default y separa permisos internos, Portal y Mobile; regeneración PDF exige review.", "FastAPI, auth, catálogo de permisos e inventario CSV", "Middleware, arranque, generador y pruebas de conformidad", "Crítico"),
        ...
        "backend/tests/test_api_access_conformity.py": ("Prueba de conformidad API", "Exige clasificación de 519 operaciones, review para regeneración PDF y coincidencia exacta entre runtime e inventario CSV.", "app.main, security.api_access e inventario CSV", "Suite backend y revisión de API", "Crítico"),
        ...
        "backend/app/routers/lab_work_orders.py": ("API OT LAB", "Expone cierre/firma grupal o individual y DELETE administrativo con permisos/scope Mobile, sin reutilizar rutas productivas.", "FastAPI, auth, schemas y servicio LAB", "MYC Mobile y pruebas LAB", "Crítico"),
        "backend/app/services/lab_work_orders.py": ("Autoridad OT LAB", "Gestiona folios, cohortes y clasificación backend conservadora de serie/identificación, audita intención y resultado e invalida replacements; conserva cierre transaccional selectivo de FieldSheets/PDFs, historia y tombstones.", "Modelos LAB, Tickets, Notifications, Communications, auditoría y PDFs", "Router LAB, MYC Mobile y pruebas", "Crítico"),
        ...
        "docs/architecture/security/API_ENDPOINT_INVENTORY_2026-08-03.csv": ("Evidencia reproducible API", "Registra 519 operaciones runtime con método, ruta, identidad, permiso, ownership y pruebas esperadas, incluida regeneración PDF con review.", "generate_api_access_inventory.py y app runtime", "Conformidad, revisión externa y CI futura", "Crítico"),
    }
    ...
    lab_operational_ux_files = {
        ...
        "backend/app/services/lab_field_sheets.py": ("Autoridad FieldSheet LAB", "Crea, edita, completa y descarta únicamente la revisión vigente editable, restaurando predecesora y estado OT.", "FieldSheet, equipo LAB, auditoría y renderer", "Router LAB, borrado OT y pruebas", "Crítico"),
        ...
        "backend/app/services/operational_tickets.py": ("Lifecycle de Tickets LAB", "Crea y resuelve Tickets, incluido reception_date_change informativo sin mutar la fecha canónica.", "SQLAlchemy, OT LAB, permisos, auditoría y Notifications", "Router móvil y revisores", "Crítico"),
        ...
        "backend/tests/test_lab_phase2_integrated_alta.py": ("Regresión alta integrada LAB", "Cubre alta/edición atómica y folio Vinculado directo o pending con notificaciones e historial.", "Pytest, TestClient, SQLAlchemy y servicios LAB", "Gate backend de equipos/folios", "Crítico"),
        "backend/tests/test_lab_phase6_field_sheet_revisions.py": ("Regresión de revisiones FieldSheet", "Verifica descarte draft/in_progress, restauración de predecesora y bloqueo de delete OT con historia.", "Pytest, modelos y servicios LAB", "Gate backend de revisiones/borrado", "Crítico"),
        ...
        "myc-mobile/src/api/error-detail.ts": ("Normalizador de errores API", "Convierte errores y 422 FastAPI en mensajes humanos y fieldErrors estructurados.", "Response y contrato ApiError", "Formularios LAB y workspace de resultados", "Crítico"),
        ...
        "myc-mobile/src/components/lab/LabEquipmentForm.tsx": ("Formulario integrado de equipo LAB", "Usa Field compartido, folio Vinculado único con ayuda por autoridad y errores corregibles por campo.", "Primitives, capacidades, cliente documental y payload", "work-orders.tsx", "Crítico"),
        ...
        "myc-mobile/src/design/primitives.tsx": ("Primitives visuales Mobile", "Centraliza botones, banners y Field con error, hint y required consistentes.", "React Native y tokens", "Pantallas/componentes MYC Mobile", "Crítico"),
    ...
    field_sheet_dsl_files = {
        ...
        "backend/app/routers/field_sheets.py": ("API FieldSheets", "Expone ciclo FieldSheet, descarga congelada y regeneración administrativa explícita del PDF vigente con field_sheets.review.", "FastAPI, auth, schemas y servicios FieldSheet", "Staff ERP y pruebas", "Crítico"),
        "backend/app/schemas/field_sheet.py": ("Contratos FieldSheet", "Tipa captura, lectura, estados y motivo obligatorio de regeneración administrativa del PDF final.", "Pydantic y schemas de referencias", "Router y servicios FieldSheet", "Alto"),
        "backend/tests/test_lab_phase5_operational_closure.py": ("Regresiones cierre LAB", "Valida cierre, permisos, reapertura directa preserve/invalidate, conservación histórica, invalidación estructural y regeneración PDF vigente con compensación de fallos.", "Pytest, TestClient, SQLite, renderer y storage", "Gate LAB", "Crítico"),
        "backend/app/services/field_sheet_pdfs.py": ("Autoridad documental PDF", "Compone el PDF canónico versionado, compensa lotes de documentos del cierre LAB y ofrece regeneración administrativa explícita auditada del final vigente.", "FieldSheet, layouts, Jinja/WeasyPrint, snapshots y storage", "Endpoints PDF, paquetes y cierres FieldSheet", "Crítico"),
