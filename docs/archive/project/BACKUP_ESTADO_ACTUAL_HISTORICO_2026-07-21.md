...
> Tipo: Histórico
>
> Autoridad: Baja; bitácora cronológica y evidencia de cortes anteriores
>
> Prevalece sobre: ninguno
>
> Reemplazado para determinar avance por: `../../project/PROJECT_STATUS.md`
>
> Reemplazado como estado operativo por: `../../BACKUP_ESTADO_ACTUAL.md`
>
> Este documento conserva fotografías sucesivas del proyecto. Sus pendientes, cierres y declaraciones de “sellado” no deben utilizarse para determinar el estado actual sin contrastarlos con la documentación canónica.

# Bitácora histórica del estado y respaldos

## Actualización 2026-07-21 — Reorganización integral de documentación

- `docs/project/DOCUMENTATION_INDEX.md` es el punto único de entrada y define la jerarquía de autoridad; `docs/project/PROJECT_STATUS.md` es la única fuente autorizada del avance actual.
- Se crearon los canónicos de alcance, flujo, reglas, decisiones, observaciones y deuda; arquitectura vigente, diseño futuro, auditorías, cierres y archivos históricos quedaron separados físicamente y rotulados.
- Los documentos redundantes de laboratorio se consolidaron sin borrar fuentes; reglas y permisos anteriores quedaron archivados después de extraer la información vigente.
- El reporte completo de movimientos, documentos vigentes/archivados, fusiones, reemplazos y 14 conflictos resueltos está en `docs/closures/DOCUMENT_REORGANIZATION_REPORT_2026-07-21.md`.
- Se regeneró y revisó `docs/PROJECT_FILE_REGISTRY.md`. No se modificó código, frontend, backend, esquema, datos ni respaldo SQL; no corresponde regenerar `backup_erp_myc_antes_prueba.sql`.

## Actualización 2026-07-21 — Auditoría integral de avance ERP MYC

- Se creó `docs/AUDITORIA_INTEGRAL_AVANCE_ERP_MYC_2026-07-21.md` como corte verificable de frontend, backend, PostgreSQL, migraciones, seguridad, integraciones, scripts, infraestructura, UX y documentación histórica. El informe clasifica 33 módulos/capacidades con los estados oficiales, detalla validaciones, observaciones resueltas/parciales/pendientes, riesgos, pendientes reales, sellos y orden recomendado hacia 1.0.
- Dictamen actual: sólo `Control Documental V1` conserva estado `SELLADO` dentro de su alcance congelado. Los bloqueadores P0 son autorización de APIs/registro/tokens/portal, duplicación material y ruta repetida en el router ETS, y autenticación de certificados duplicada fuera de Calidad.
- Evidencia ejecutada: 63 pruebas backend correctas; build Vite correcto con advertencia de chunk principal de 852.38 kB; PostgreSQL local con 55 tablas, 151,229 registros SAT y `alembic current = fd5e6f7a8b9c (head)`; revisión visual acotada de Login y laboratorio de Hojas de Campo; `scripts/myc doctor` confirmó un defecto de falsos negativos en su función `check` y discrepancia de puerto frontend.
- `AGENTS.md` incorpora el procedimiento oficial para futuras auditorías integrales sin sustituir las reglas existentes. `scripts/generate_project_file_registry.py` reconoce el nuevo informe, describe la responsabilidad ampliada de `AGENTS.md` y actualiza la fecha reproducible del inventario.
- No se modificó código de producto, esquema ni datos de negocio. No corresponde regenerar `backup_erp_myc_antes_prueba.sql`; Alembic permanece en `fd5e6f7a8b9c (head)`.
