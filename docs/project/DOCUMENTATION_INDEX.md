...
| [`../architecture/MOBILE_DEVELOPER_AUTHORITY.md`](../architecture/MOBILE_DEVELOPER_AUTHORITY.md) | DEV-0 Developer Authority de MYC Mobile: `DeveloperSession` biométrica sobre BIOMETRIC-2, política Developer explícita (`"*"` no la concede), una sola sesión activa por dispositivo (supersession), binding por `MobileAuthSession`, reconciliación Mobile por lifecycle y contrato futuro `ShellSession`; EN REVISIÓN, sin Shell Broker. |
| [`../architecture/MOBILE_DEVELOPER_BROKER.md`](../architecture/MOBILE_DEVELOPER_BROKER.md) | Developer Broker. DEV-1A (mergeado, PR #8): frontera FastAPI ↔ Broker, contrato `myc.developer-broker` v1, HMAC-SHA256 canónico con anti-replay y `GET /developer/broker/health` gateado por DEV-0. DEV-1B (EN REVISIÓN, pendiente de validación en Windows real): adapter Windows Named Pipe (pywin32), framing con prefijo de longitud, DACL explícita de dos identidades, verificación de identidad del servidor antes de escribir y host independiente `python -m app.developer_broker.host`; sin shell, sin ejecución de procesos, sin puertos. |
