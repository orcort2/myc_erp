...
| [`../architecture/MOBILE_DEVELOPER_BROKER.md`](../architecture/MOBILE_DEVELOPER_BROKER.md) | DEV-1A Developer Broker: frontera FastAPI ↔ Broker, contrato versionado `myc.developer-broker` v1, HMAC-SHA256 canónico con anti-replay, puerto de transporte (Named Pipe Windows pendiente, in-memory sólo para pruebas) y `GET /developer/broker/health` gateado por DEV-0; EN REVISIÓN, sin shell ni ejecución de procesos. |
