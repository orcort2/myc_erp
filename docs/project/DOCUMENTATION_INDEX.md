...
| [`../architecture/MOBILE_SECURITY_CONTEXT.md`](../architecture/MOBILE_SECURITY_CONTEXT.md) | Identidad Mobile internal/client, BIOMETRIC-1: dispositivos independientes, sesiones persistentes, refresh opaco rotatorio, reuse, logout, single-flight y transición legacy temporal; BIOMETRIC-2: login biométrico device-bound con credencial opaca separada; RBAC, scope, realtime y push. |
| [`../architecture/MOBILE_DEVELOPER_AUTHORITY.md`](../architecture/MOBILE_DEVELOPER_AUTHORITY.md) | DEV-0 Developer Authority de MYC Mobile: `DeveloperSession` biométrica sobre BIOMETRIC-2, política Developer explícita (`"*"` no la concede), una sola sesión activa por dispositivo (supersession), binding por `MobileAuthSession`, reconciliación Mobile por lifecycle y contrato futuro `ShellSession`; mergeado (PR #7), sin Shell. |
| [`../architecture/MOBILE_DEVELOPER_BROKER.md`](../architecture/MOBILE_DEVELOPER_BROKER.md) | Developer Broker. DEV-1A (mergeado, PR #8): frontera FastAPI ↔ Broker, contrato `myc.developer-broker` v1, HMAC-SHA256 canónico con anti-replay y `GET /developer/broker/health` gateado por DEV-0. DEV-1B (mergeado, PR #9; validado en Windows real 11 passed/1 skipped/0 failed): adapter Windows Named Pipe (pywin32), framing con prefijo de longitud, DACL explícita de dos identidades, verificación de identidad del servidor antes de escribir y host independiente `python -m app.developer_broker.host`. DEV-1C (EN REVISIÓN, instalación en Windows pendiente): servicio WinSW `MYCDeveloperBroker` bajo la cuenta virtual `NT SERVICE\MYCDeveloperBroker`, ACL mínimas, modelo de secreto, endurecimiento de `C:\MYC\Services`, preflight, rollback y runbook (activos en `deploy/windows/developer-broker/`, que no son documentación). Sin shell, sin ejecución de procesos, sin puertos. |
...
| [`../closures/BIOMETRIC_1_MOBILE_SESSION_AUTHORITY.md`](../closures/BIOMETRIC_1_MOBILE_SESSION_AUTHORITY.md) | Implementación y validación BIOMETRIC-1: dispositivo, sesión, rotación, reuse, logout, migración legacy y single-flight; no sella Mobile. |
| [`../closures/BIOMETRIC_2_BIOMETRIC_LOGIN.md`](../closures/BIOMETRIC_2_BIOMETRIC_LOGIN.md) | Implementación y validación BIOMETRIC-2: login biométrico Face ID/Touch ID/Android sobre la Mobile Session Authority, credencial opaca separada, storage local perfil/credencial y corrección de overscroll en Home técnica; no sella Mobile. |
...
| [`../closures/LAB_OPERATIONAL_UX_CLOSURE_2026-09-03.md`](../closures/LAB_OPERATIONAL_UX_CLOSURE_2026-09-03.md) | Cierre técnico de folio Vinculado, descarte seguro, fecha de recepción, Ticket informativo, errores por campo, calendario y canon visual OT LAB. |
