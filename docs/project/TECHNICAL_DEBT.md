...
| TD-014 | P0 | La “excepción” ETS todavía ejecuta cambio de etapa y resincronización al solicitarla; no separa `requested`, `approved` y `executed`, y admite actor opcional. Fase 14 retiró equipo adicional de ese patrón mediante un vertical gobernado, pero los demás casos siguen abiertos. | modelos/schemas/servicios ETS, routers, permisos, Invoice y `register_service_order_exception` | Migrar cada excepción restante a un vertical explícito con autorización, revalidación y servicio canónico; nunca ampliar el bypass genérico. Cancelación/sustitución CFDI conserva flujo fiscal propio. |
