...
| ADR-004 | 2026-07-10 | Las OT se agrupan en bloques de máximo 10 equipos y las firmas se capturan por ciclos de OT pendientes. | Modelos, servicios vigentes y cierre de reorganización documental | Una OT posterior no invalida firmas anteriores y requiere ciclo nuevo. |
...
| ADR-053 | 2026-09-02 | El DSL avanzado de Hojas de Campo amplía snapshots y el renderer canónico v1 con defaults backward-compatible; la matriz de headers y su posición lógica, layout seguro, disposición tipada de firmas y perfiles organizacionales se normalizan en Python y Jinja sólo presenta. Mobile deriva posiciones absolutas desde la misma matriz para representar rowspan real. | [`../architecture/FIELD_SHEET_PDF_RENDERER.md`](../architecture/FIELD_SHEET_PDF_RENDERER.md) y pruebas Fases 4+5 | No hay migración, cambio de `FieldSheetResult`, renderer por template ni reinterpretación material de snapshots v1. Firmas conserva grid horizontal derivado por default y permite vertical/campos posteriores allowlisted; CAPYMET queda aislado del contacto/logo MYC. |
| ADR-088 | 2026-09-03 | Fase 6A.1 materializa Temperatura y Presión como definiciones MYC versionadas sobre el motor/renderer vigente, desde sus FCA-30 originales. La clave histórica de Presión evoluciona a versión 3 y Temperatura a versión 2 sin migración ni reescritura; el snapshot persistido sigue siendo autoridad. | [`../architecture/FIELD_SHEET_TEMPLATE_INVENTORY_PHASE_6.md`](../architecture/FIELD_SHEET_TEMPLATE_INVENTORY_PHASE_6.md) y evidencia PDF Fase 6A.1 | Los textos y geometrías documentales se declaran con el DSL; metadata de equipo sólo descubre/recomienda y no crea compatibilidad productiva ni ramas por plantilla. Estado: EN REVISIÓN. |
...
| ADR-015 | 2026-07-10 | Control Documental V1 queda sellado con el diseñador deshabilitado. | Cierre V1 y auditoría 2026-07-21 | El diseñador general no es pendiente de V1. |
...
| ADR-081 | 2026-08-18 | La identidad operativa se persiste como `operational_category`; `item_type` conserva sólo Producto/Servicio comercial-fiscal y no implica ni bloquea `sale`. `category`, `commodity` y `calibration_scope` conservan responsabilidades distintas. La cotización es autoridad histórica mediante snapshot esquema 2 y no se refresca al editar el mismo concepto. Sólo sustituir explícitamente el catálogo genera snapshot nuevo. | [`../architecture/OPERATIONAL_SERVICE_IDENTITY.md`](../architecture/OPERATIONAL_SERVICE_IDENTITY.md), migraciones `a8c0e2f4b6d8`/`e2a4c6d8f0b1` y suite focal | Elimina inferencias por tipo/nombre/descripción, conserva compatibilidad estructurada exacta y no reinterpreta históricos. Queda **EN REVISIÓN**. |
| ADR-082 | 2026-08-25 | `accepted` es el evento institucional que materializa el ETS dentro de la misma transacción backend. La Cotización se bloquea, `quotation_id` aporta idempotencia y el frontend sólo abre el ETS existente. Verificación exige Master genérico válido en altas/actualizaciones; el legacy nulo permanece legible pero bloquea materialización hasta sustitución explícita. | `quotations.py`, `service_orders.py`, `catalog_items.py`, Catálogo/ETS web y suites focales | Elimina estado aprobado sin ETS por fallo parcial y traslada el faltante de Master desde Captura a la frontera institucional correcta, sin migración ni reinterpretación histórica. Queda **EN REVISIÓN**. |
| ADR-083 | 2026-08-26 | MYC Mobile usa una autoridad de sesión propia que compone `User` con RBAC interno o `ClientPortalMembership` y RBAC externo. `mobile.access` no implica operaciones. Cliente queda limitado por `client_id` resuelto en DB y máximo una membership activa, reforzada por índice parcial. Sólo LAB temporal se expone; rutas productivas quedan internas. | [`../architecture/MOBILE_SECURITY_CONTEXT.md`](../architecture/MOBILE_SECURITY_CONTEXT.md), migración `d6f2a4c8e0b1` y suites Mobile | Evita técnicos ficticios, tokens cliente como internos, IDOR y mezcla Portal/Mobile. Conserva sesiones internas emitidas previamente como compatibilidad temporal y no abre folios. Queda **EN REVISIÓN**. |
...
contrato LAB vigente. Originalmente `root_work_order_id` era la única identidad
local; ADR-086 la conserva para cierre grupal y usa el ID de OT para cierre
individual. El contexto se revalida antes del POST y cualquier cambio crea un
borrador vacío sin caché de retorno.
La UI toma tarjeta, jerarquía, pasos y feedback del ERP como referencia visual,
sin importar código web ni alterar el motor de firma o reapertura.
