...
| ADR-068 | 2026-08-18 | La identidad operativa se persiste como `operational_category`; `category`, `commodity` y `calibration_scope` conservan responsabilidades distintas. La cotización es autoridad histórica mediante snapshot esquema 2 y no se refresca al editar el mismo concepto. Sólo sustituir explícitamente el catálogo genera snapshot nuevo. | [`../architecture/OPERATIONAL_SERVICE_IDENTITY.md`](../architecture/OPERATIONAL_SERVICE_IDENTITY.md), migración `a8c0e2f4b6d8` y suite focal | Elimina inferencia por nombres y el fallback accidental a Servicio General sin abrir workflows técnicos ni mover la selección de Hojas de Campo. Queda **EN REVISIÓN**. |
...
| ADR-076 | 2026-08-18 | ETS Venta usa una proyección normalizada materializada sólo desde el snapshot comercial. Las unidades identificables reutilizan `ServiceUnit`/`ServiceStage` y, con calibración, un único `Equipment`. La compatibilidad histórica se activa mediante acción explícita, no por lectura. | [`../architecture/SALE_ETS_EXECUTION.md`](../architecture/SALE_ETS_EXECUTION.md), migración `b9d1f3a5c7e9` y suite Venta | Conserva parciales, garantía y entrega multicanal sin consultar catálogo vigente ni abrir workflows ajenos. Queda **EN REVISIÓN**. |
