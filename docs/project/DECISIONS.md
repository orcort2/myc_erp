...
| ADR-053 | 2026-09-02 | El DSL avanzado de Hojas de Campo amplía snapshots y el renderer canónico v1 con defaults backward-compatible; la matriz de headers, layout seguro y perfiles MYC/CAPYMET se normalizan en Python y Jinja sólo presenta. | [`../architecture/FIELD_SHEET_PDF_RENDERER.md`](../architecture/FIELD_SHEET_PDF_RENDERER.md) y pruebas Fases 4+5 | No hay migración, cambio de `FieldSheetResult`, renderer por template ni reinterpretación material de snapshots v1; CAPYMET queda con fallback textual hasta recibir asset oficial. |
