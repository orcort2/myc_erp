...
| BR-060 | Hojas de Campo | La geometría de captura/PDF pertenece al snapshot declarativo: headers multinivel deben cubrir exactamente la tabla sin overlap y cada `column_key` debe coincidir con su posición lógica (`__row_number__` en 0 y resultados en orden declarado); `row_labels` no reemplaza `row_number`. Layout, identidad visual y cortes se resuelven mediante propiedades tipadas/allowlisted, nunca mediante HTML, CSS, Jinja, URLs, paths ni branches por template. CAPYMET no hereda contacto ni logo MYC. | `field_sheet_layouts.py`, schemas, renderer canónico y pruebas DSL | 2026-09-02 |
