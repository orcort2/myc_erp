...
| BR-057 | OT LAB temporal | Una OT LAB admite máximo 10 equipos y folio backend 6400–6999. Una adicional sólo nace desde la última OT `draft` llena, hereda generales y conserva cadena por ID. El grupo (`root_work_order_id`) es parentesco histórico; la sesión es una cohorte de cierre versionada. El cierre grupal firma sólo las OT abiertas participantes y exige equipo en todas; el individual firma sólo la OT elegida y exige equipo únicamente en ella. Cada modalidad conserva doble firma. Las completadas no se reparentan, renumeran, regeneran ni invalidan al editar/cerrar hermanas. El borrador móvil se liga a raíz para grupo y a OT para individual, sin reutilizarse entre contextos. `signature_required=false` es normal antes de la primera firma y no impide su POST. Cada OT conserva PDF individual y toda retirada exige exportación íntegra verificada previa. | [`../architecture/LAB_WORK_ORDERS.md`](../architecture/LAB_WORK_ORDERS.md) y suites LAB/móvil | 2026-08-27 |
