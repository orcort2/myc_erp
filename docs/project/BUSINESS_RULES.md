...
| BR-050 | Hojas de Campo LAB | Una FieldSheet persistida se interpreta exclusivamente con su `template_definition_json`; sólo una revisión LAB puede ser vigente por equipo y una revisión histórica completed no cambia estado, PDF ni SHA. Una clave no soportada devuelve error explícito. | modelos/servicios LAB, migración `d7c297902425` y cierre Fase 6 | 2026-09-01 |
| BR-060 | Hojas de Campo | La geometría de captura/PDF pertenece al snapshot declarativo: headers multinivel deben cubrir exactamente la tabla sin overlap y cada `column_key` debe coincidir con su posición lógica (`__row_number__` en 0 y resultados en orden declarado); `row_labels` no reemplaza `row_number`. Layout, identidad visual, cortes y disposición de firmas se resuelven mediante propiedades tipadas/allowlisted, nunca mediante HTML, CSS, Jinja, URLs, paths ni branches por template. Los campos posteriores a firmas sólo aceptan claves reconocidas y reutilizan la resolución canónica de label/valor. CAPYMET no hereda contacto ni logo MYC. | `field_sheet_layouts.py`, schemas, renderer canónico y pruebas DSL | 2026-09-02 |
| BR-070 | Catálogo Hojas de Campo | Organización + magnitud + variante es el eje del catálogo; equipo no lo es. `supported_equipment` y `search_aliases` son exclusivamente informativos/de búsqueda y nunca pueden bloquear selección manual, creación, autorización, asignación, completion, lifecycle o PDF. | motor de plantillas, selector Mobile y pruebas Fase 6A.1 | 2026-09-03 |
...
| BR-057 | OT LAB temporal | Una OT LAB admite máximo 10 equipos y folio backend 6400–6999. Una adicional sólo nace desde la última OT `draft` llena, hereda generales y conserva cadena por ID. El grupo (`root_work_order_id`) es parentesco histórico; la sesión versionada acredita recepción técnico+cliente y puede ser grupal o individual. Sólo ambas firmas válidas crean sesión y producen `received_signed`; la primera FieldSheet produce `in_progress`, todas las hojas requeridas completas producen `ready_to_close` y el cierre produce `completed` sin otra firma. Cada FieldSheet congela la sesión vigente al crearla. `ready_for_signatures` es sólo legacy. La recepción firmada es inmutable salvo reapertura aprobada; las completadas no se reparentan, renumeran, regeneran ni invalidan al operar hermanas. El borrador móvil se liga a raíz para grupo y a OT para individual, sin reutilizarse entre contextos. `signature_required=false` es normal antes de la primera firma y no impide su POST. Cada OT conserva PDF individual y toda retirada exige exportación íntegra verificada previa. | [`../architecture/LAB_WORK_ORDERS.md`](../architecture/LAB_WORK_ORDERS.md) y suites LAB/móvil | 2026-09-01 |
...
| BR-059 | OT LAB temporal | Sólo `lab_work_orders.delete` autoriza eliminar una OT LAB individual. La eliminación reutiliza el descarte de FieldSheet cuando todas sus hojas son revisiones vigentes `draft`/`in_progress`; cualquier hoja `completed` o revisión histórica bloquea el hard delete y obliga a conservar trazabilidad. Se borran OT, equipos, PDF/revisiones/tickets exclusivos y se conserva cualquier hermana, sesión de firma, ticket, revisión o notificación todavía compartida. Al retirar raíz o eslabón intermedio se repara y compacta la cadena dentro de la misma transacción. Los folios eliminados permanecen consumidos y el secuenciador nunca retrocede. | [`../architecture/LAB_WORK_ORDERS.md`](../architecture/LAB_WORK_ORDERS.md), servicio y suite LAB | 2026-09-03 |
...
Técnico internal posee `lab_work_order_groups.create` y puede crear el grupo
directo. Ese permiso no concede `requests.read`, `requests.claim` ni
